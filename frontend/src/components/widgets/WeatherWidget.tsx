...
const weatherDescriptions: Record<number, { description: string; icon: JSX.Element }> = {
    ...
    81: { description: 'Moderate Rain Showers', icon: <BsCloudRainHeavyFill style={{ fontSize: '2.5rem' }}/> },
    82: { description: 'Heavy Rain Showers', icon: <BsCloudRainHeavyFill style={{ fontSize: '2.5rem' }}/> },
...
export const WeatherWidget: React.FC = () => {
    ...
    const renderWeatherItem = () => {
        return weatherData && Array.from({ length: forecastDays }, (_, index) => {
            ...
            const weatherInfo = weatherDescriptions[weatherCode] || { description: 'Unknown', icon: <BsCloudSunFill style={{ fontSize: '2.5rem' }} /> };
